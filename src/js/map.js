...
var HOME_PATH = 'https://seoulshelter.live';
let markers = [];
...
async function fetchRainfallData() {
  ...
  const requests = districts.map(async district => {
      ...
      const apiUrl = `http://openapi.seoul.go.kr:8088/4c6d47686168696539317776437144/xml/ListRainfallService/1/5/${encodedDistrict}`;
      const proxyUrl = `https://proxy.seoulshelter.live/${apiUrl}`;
...
document.querySelector('#shelter-button').addEventListener('click', async function(e) {
  ...
  // 대피소 마커 생성
  const urls = [
    'http://openapi.seoul.go.kr:8088/69614375656869653131315a4246614a/xml/TbEqkShelter/1/1000',
    'http://openapi.seoul.go.kr:8088/69614375656869653131315a4246614a/xml/TbEqkShelter/1001/2000'
  ];
...
async function getCoordinates(url) {
  const proxyUrl = `https://proxy.seoulshelter.live/${url}`;
  const response = await fetch(proxyUrl, {
    headers: {
